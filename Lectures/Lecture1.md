...
Если аргумент функции не важен в данном уравнении, можно использовать символ нижнего подчеркивания. Пример -- функция двух аргументов const, которая возвращает свой первый аргумент: второй аргумент сдесь называть не нужно, потому что он не используется.
...
Порядок проверки так же сверху вниз -- если не прошло первое охранное выражение, то переходим к следующему.
...
В частности, используя `where` можно определять локальные функции, т.е. те, которые нужны только внутри данной. Пример:
