...
# [Лекция 1 -- выражения и функции]{.class lang="ru-RU"}

:::::{.class lang="ru-RU"}
# Парадигма:

	Программа на Haskell -- это набор выражений.
	Выполнить программу -- редуцировать выражения до тех пор пока можно. 
...
Выражение1 и Выражение2 должны быть одного **типа**.
Если логическое выражение равно (т.е. редуцируется в) `True`, то выполняется первая ветка, иначе вторая.
Вычисления производятся *лениво*, т.е. то выражение которое не требует немедленного подсчета, не будет вычисляться.
...
error "Сообщение" --- завершает программу в тот момент, когда требуется вычислить это выражение, с сообщением об ошибке.

undefined --- завершает программу в тот момент, когда требуется вычислить это выражение, с стандартным сообщением об ошибке. Подходит для еще нереализованных функций.
...
Так как `(1 > 0) == True`, то ошибки не случится, так как Haskell не будет вычислять часть после `else` 
...
Функция -- это именованное выражение. Функция может зависеть от аргументов (любого числа). Примерение функции к аргументам пишем без знаков препинания (через пробел):
...
Если аргумент функции не важен в данном уравнении, можно использовать 
символ нижнего подчеркивания. Пример -- функция двух аргументов const,
которая возвращает свой первый аргумент: второй аргумент сдесь называть
не нужно, потому что он не используется.
:::::

```haskell

-- const is a standart function, so write it with '.
const' a _ = a
...
Работает внутри функций -- как let ... in, но наоборот.
Не является выражением, работает только в теле функции.
:::::
