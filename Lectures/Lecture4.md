...
Мы видим, что вторая функция сломалась. Почему? Потому что мы не умеем выводить в консоль константы нашего типа, так как для этого тип должен быть представителем класса типов `Show`. К счастью, для таких простых классов типов существует автоматическая доставка функций. Для этого можно использовать ключевое слово `deriving`:
